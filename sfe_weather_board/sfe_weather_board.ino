...
long altitude = 1596; // 1596 default - fixed weather station altitude in meters, for relative (sea level) pressure measurement
...
// loops forever after setup() ends
{
  ...
  switch (pressure_units)
  {
    ...
    case PSI:
      ...
      break;
    case MMHG:
      BMP085_pressure = BMP085_pressure / 1.33322368;
      break;
    default:
    ...
    case ANSI: // this data_format neatly formats the data on an ANSI terminal, and has pass/fail values for testing
    {
      ...
      switch (pressure_units) // change decimal point for different units
      {
        ...
        case MMHG:
          ...
          Serial.print(" mm Hg ");
          ansiTab();
          if ((BMP085_pressure > 30) && (BMP085_pressure < 900)) pass(); else fail();
          break;
      }
...
// provide the user a way to modify settings, and save those settings to EEPROM to survive reboot / shutdown
{
  ...
  while (!done)
  {
    ...
    switch (pressure_units)
    {
      ...
      case MMHG: Serial.print("mm Hg"); break;
    }
    Serial.println(")");
